...
const Tariff = () => {
  return (
    <div className="flex tariffs flex-btw">
        {_items.map((item) => {          
          let button = <Button type="tariff" label="Подробнее" style="tariffs__btn" />;
          let border;
          let badge = <hr className="tariffs__inactive" />;

          if (item.tariff == `Beginner`) { //пока делаем текущий тариф хардкодом
            border = `2px solid ` + item.color;
            badge = <div className="tariffs__current">Текущий тариф</div>
            button = <Button type="tariff" label="Перейти в личный кабинет" style="tariffs__btn btn-grey" />;
          } 
          ...
          return (
            <div className="tariffs__card" style={{border: `${border}`}} key={item.id}>
              <div className="tariffs__top" style={{backgroundColor: `${item.color}`, backgroundImage: `url("${item.image}")`, color: `${item.colorHeader}` }}>
                <h3>{item.tariff}</h3>
                <p>{item.desc}</p>
              </div>
              {badge}
              <div className="tariffs__middle">                
                <div className="flex tariffs__prices">
                  <p className="tariffs__prices-actual">
                  {item.price} ₽
                  </p>
                  <p className="tariffs__prices-grey">
                    {item.price_strike} ₽
                  </p>
                </div>
